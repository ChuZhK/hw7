...
public class MainController : MonoBehaviour {
    ...
    //public void updateF()
    //{
    //    Vector3 world1=new Vector3();//物体要移动到的位置 （世界坐标系）
    //    Vector3 targetposition = Camera.main.WorldToScreenPoint(player.transform.position);
    //    Vector3 mouseposition = Input.mousePosition;//鼠标在屏幕上的位置坐标
    //    if (targetposition.y != mouseposition.y)
    //    {
    //        player.transform.Rotate(0, (mouseposition.y - targetposition.y) * rotateSpeed * Time.deltaTime, 0);
